...
export default function TeamShowcase({ members = DEFAULT_MEMBERS }: TeamShowcaseProps) {
  ...
  return (
    <div className="flex flex-col md:flex-row items-start gap-8 md:gap-16 lg:gap-24 select-none w-full max-w-[1200px] mx-auto py-8 px-4 md:px-6 font-sans">
      {/* ── Left: photo grid ── */}
      <div className="flex gap-4 md:gap-8 lg:gap-10 flex-shrink-0 overflow-x-auto pb-1 md:pb-0 mx-auto md:mx-0">
        {/* Column 1 */}
        <div className="flex flex-col gap-4 md:gap-8 lg:gap-10">
          {col1.map((member) => (
            <PhotoCard
              key={member.id}
              member={member}
              className="w-[140px] h-[160px] sm:w-[180px] sm:h-[200px] md:w-[230px] md:h-[250px]"
              hoveredId={hoveredId}
              onHover={handleHover}
            />
          ))}
        ...
        {/* Column 2 */}
        <div className="flex flex-col gap-4 md:gap-8 lg:gap-10 mt-[48px] sm:mt-[60px] md:mt-[78px]">
          {col2.map((member) => (
            <PhotoCard
              key={member.id}
              member={member}
              className="w-[150px] h-[170px] sm:w-[195px] sm:h-[215px] md:w-[245px] md:h-[265px]"
              hoveredId={hoveredId}
              onHover={handleHover}
            />
          ))}
